...
export default class Bear {
  constructor(data) {
    ...
    this.sprite = scene.matter.add.sprite(x, y, 'bear', 'bear_idle_1');
    ...
    // Làm cho Bear đi xuyên qua cây, đá và các vật cản khác
    // Chỉ va chạm với player
    this.sprite.setCollisionCategory(0x0002); // Category riêng cho Bear
  ...
  updateHealthBar() {
    ...
    // Cập nhật vị trí health bar theo bear
  ...
  setupAnimations() {
    if (this.scene.anims.exists('bear_idle')) {
      this.sprite.play('bear_idle');
    } else {
      console.warn('Bear animations not loaded yet');
    }
  }
  ...
  update() {
    ...
    if (distance <= this.meleeRange) {
      ...
      if (this.scene.anims.exists('bear_idle')) {
        this.sprite.play('bear_idle', true);
      }
  ...
  idleWalk(target) {
    ...
    if (this.scene.anims.exists('bear_walk')) {
      this.sprite.play('bear_walk', true);
    }
